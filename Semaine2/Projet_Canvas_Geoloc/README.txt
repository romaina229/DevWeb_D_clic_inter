...
---
9. Modifications appliquées (par rapport à la version d'origine)

Voici les changements que j'ai effectués pour intégrer une carte interactive et améliorer la géolocalisation :

- Fichier `index_API_de_Géolocalisation.html` :
  - Remplacé le `<canvas>` utilisé pour la position par une div `<div id="map"></div>` destinée à contenir une carte Leaflet.
  - Ajouté les inclusions Leaflet (CSS et JS) via CDN.
  - Ajouté un élément `<div>position est <span id="position"></span></div>` pour afficher le nom lisible de la position.

- Fichier `asset/style1.css` :
  - Ajout du style `#map` (taille 400×300, bordure blanche, coins arrondis) pour rendre la carte visible et cohérente avec le design.

- Fichier `js/js1.js` :
  - Remplacé le code de dessin sur canvas par l'initialisation d'une carte Leaflet centrée par défaut sur Paris.
  - Ajout d'une couche de tuiles OpenStreetMap (affiche les noms des villes/pays).
  - Ajout de la gestion d'un marqueur qui se positionne sur la position actuelle et qui est sauvegardée dans `localStorage`.
  - Implémentation d'un reverse geocoding via l'API Nominatim (OpenStreetMap) pour obtenir un nom humainement lisible (ex. "Rue, Ville, Pays") et l'afficher dans la popup du marqueur et dans le span `#position`.
